...
class Polygon:
    ...
    def cut(self, line):
        ...
        if len(intersections) > 2: # removing duplicate intersections in case on cut on vertex
            intersections = [intersec for i,intersec in enumerate(intersections) 
                                      if intersec[1] != intersections[(i+1)%len(intersections)]]
        assert len(intersections) == 2

        i1,i2 = (intersections[0], intersections[1]) 
        assert i1[0] != i2[0]
        if i2[0] < i1[0]: i1,i2 = i2, i1 # ordered pair

        s1 = [v for v in self.vertices[i1+1:i2+1]]
        l_vs =  [i1[1]] + s1 + [i2[1]]

        s2 = [v for v in self.vertices if v not in s1]
        r_vs = s2[:i1[0]] + [i1[1], i2[1]] + s2[i1[0]:]
...
class Segment:
    ...
    def __contains__(self, vec):
        ...
        if x1 == x2: # vertical line
            return x2 == x3 and (y1 <= y3 <= y2 or y1 >= y3 >= y2)
        if y1 == y2: # horizontal line
            return y2 == y3 and (x1 <= x3 <= x2 or x1 >= x3 >= x2)
    ...
    def intersection(self, line):
        ...
        if geq(det,0.0):  # lines are parallel
            ...
            if self.p1 in line: return self.p1 
            if self.p2 in line: return self.p2
            if line.p1 in self: return line.p1
            if line.p2 in self: return line.p2
        ...
        if not self.is_line and (ua < 0 or ua > 1): 
            return None # out of range
        ub = ((x2-x1)*(y1-y3) - (y2-y1)*(x1-x3)) / denom
        if not line.is_line and (ub < 0 or ub > 1): 
            return None # out of range
